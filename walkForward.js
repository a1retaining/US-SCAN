...
const app=express(); const PORT=process.env.PORT||10000; const VERSION="5.3.0-self-training-proof-engine"; app.use(express.json({limit:"1mb"})); app.use(express.static(path.join(__dirname,"../public"))); let lastUniverse=null,lastUniverseTime=0;
async function getUniverse(force=false){if(!force&&lastUniverse&&Date.now()-lastUniverseTime<180000)return lastUniverse;const db=readDb();const universe=await fetchUniverse(DEFAULT_SYMBOLS,70,"3y");lastUniverse=universe;lastUniverseTime=Date.now();for(const [symbol,bars] of Object.entries(universe.barsBySymbol)){db.historicalBars[symbol]={range:"3y",interval:"1d",updatedAt:new Date().toISOString(),bars};}db.historicalSnapshots.unshift({time:new Date().toISOString(),range:"3y",interval:"1d",symbols:Object.keys(universe.barsBySymbol),errorCount:universe.errors.length});db.historicalSnapshots=db.historicalSnapshots.slice(0,500);writeDb(db);return universe;}
